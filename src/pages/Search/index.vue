...
export default {
  ...
  data() {
    return {
      ...
      searchParams: {
        ...
        // 关键字
        keyword: "",
        // 排序 初始的状态应该是综合|降序（1:desc）  1(综合) 2(价格) desc(降序) asc(升序)
        order: "1:asc",
        // 分页器用的：代表当前是第几页
        pageNo: 1,
        // 代表每一页展示数据个数
  ...
  methods: {
    ...
    removeCategoryName() {
      ...
      this.getData();
      // 地址栏也需要改：进行路由跳转
      // 严谨：本意是删除query参数，如果路径中出现params不应该删除，路由跳转的时候应该带着
    ...
    // 排序的操作
    changeOrder(flag) {
      // flag形参：它是一个标记，代表用户点击的是综合（1）价格（2）[用户点击的时候传进来的]
  ...
  watch: {
    ...
    $route(newValue, oldValue) {
      ...
      // 再次发起ajax请求
      this.getData();
      // 每一次请求完毕，应该把相应的1，2，3级分类的id清空，让它接收下一次的1，2，3级id
      // 分类名字和关键字不用清空：因为每一次路由发生变化时，都会赋予新的数据
      this.searchParams.category1Id = undefined;
      this.searchParams.category2Id = undefined;
      this.searchParams.category3Id = undefined;
    },
