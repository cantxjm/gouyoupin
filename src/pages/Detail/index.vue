        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 以前的路由跳转：从A路由跳转到B路由，这里在加入购物车，进行路由跳转时，发请求
...
export default {
  ...
  methods: {
    ...
    async addShopcar() {
      // 派发action
      // 1.发请求---将产品加入到数据库（通知到服务器）
      // 你需要知道这次请求成功还是失败，成功跳转，失败给用户提示
      try {
        ...
        // 一些简单的数据skuNum，通过query形式给组件传递过去
        // 产品信息数据【比较复杂：skuInfo】，通过会话存储（不持久化，会话结束数据再消失）
        // 本地存储|会话存储，一般储存的是字符串
